...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 2, -1, -1):
            ...
            # dropout
            if self.use_dropout:
               curr_dout = dropout_backward(curr_dout, do_cache)
            # relu
            curr_dout = relu_backward(curr_dout, relu_cache)
            # batch/layer norm
            dgamma, dbeta = None, None
            if self.normalization == 'batchnorm':
               curr_dout, dgamma, dbeta = batchnorm_backward_alt(curr_dout, bn_cache)   
            elif self.normalization == 'layernorm':
               curr_dout, dgamma, dbeta = layernorm_backward(curr_dout, bn_cache)
            # affine layer
            dx, dw, db = affine_backward(curr_dout, fc_cache)
